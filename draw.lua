...
function Menu.Notify(message, notifType, duration)
...
function Menu.DrawNotifications()
    ...
    local barWidth = 3
    local startX = screenWidth - notifWidth - 20
    local startY = 20

    local i = 1
    while i <= #Menu.Notifications do
        local n = Menu.Notifications[i]

        -- Tick timer down
        n.timer = n.timer - (dt * 1000)

        -- Fade in, then fade out in last 20% of lifetime
        if n.timer > n.duration * 0.2 then
            n.alpha = math.min(1.0, n.alpha + fadeSpeed)
        else
            n.alpha = math.max(0.0, n.alpha - fadeSpeed)
        end

        -- Remove once fully invisible and expired
        if n.timer <= 0 and n.alpha <= 0 then
            table.remove(Menu.Notifications, i)
        else
            local y = startY + (i - 1) * (notifHeight + gap)
            local a = n.alpha

            -- Dark background card
            if Susano and Susano.DrawRectFilled then
                Susano.DrawRectFilled(startX, y, notifWidth, notifHeight, 0.05, 0.05, 0.05, 0.92 * a, cornerRadius)
            else
                Menu.DrawRoundedRect(startX, y, notifWidth, notifHeight, 13, 13, 13, math.floor(235 * a), cornerRadius)
            end

            -- Colored left accent bar
            local barR, barG, barB
            if n.notifType == "success" then
                barR, barG, barB = 0.18, 0.85, 0.4
            elseif n.notifType == "error" then
                barR, barG, barB = 0.95, 0.2, 0.2
            else -- info: use current theme accent
                barR = (Menu.Colors.SelectedBg and Menu.Colors.SelectedBg.r or 148) / 255.0
                barG = (Menu.Colors.SelectedBg and Menu.Colors.SelectedBg.g or 0)   / 255.0
                barB = (Menu.Colors.SelectedBg and Menu.Colors.SelectedBg.b or 211) / 255.0
            end

            if Susano and Susano.DrawRectFilled then
                Susano.DrawRectFilled(startX, y, barWidth, notifHeight, barR, barG, barB, 1.0 * a, cornerRadius)
            else
                Menu.DrawRect(startX, y, barWidth, notifHeight,
                    math.floor(barR * 255), math.floor(barG * 255), math.floor(barB * 255), math.floor(255 * a))
            end

            -- Message text
            local textSize = 14
            local textX = startX + barWidth + padding
            local textY = y + (notifHeight / 2) - (textSize / 2)
            Menu.DrawText(textX, textY, n.message, textSize, 1.0, 1.0, 1.0, 1.0 * a)
